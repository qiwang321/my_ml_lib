...
float* weights_up[NUM_LAYER + 1]; //space storing the up weights (first is not used)
float* weights_down[NUM_LAYER + 1]; //space storing the down weights (first is not used)
...
float yita_w = 0.1, yita_bv = 0.1, yita_bh = 0.1,
		yita_wt = 0.001, yita_bvt = 0.001, yita_bht = 0.001; // learning rates
float mu = 0.4, reg = 0.0002;
...
int main() {
	...
	for (int k = 1; k < NUM_LAYER + 1; k++) {
		weights_up[k] = (float *)  malloc(nodes_layer[k-1] * nodes_layer[k] * sizeof(float));
		weights_down[k] = (float *)  malloc(nodes_layer[k-1] * nodes_layer[k] * sizeof(float));
		...
		for (int j = 0; j < nodes_layer[k-1] * nodes_layer[k]; j++) {
			weights_up[k][j] = 0.1 * ((float)rand()/RAND_MAX * 2 - 1);
			weights_down[k][j] = 0.1 * ((float)rand()/RAND_MAX * 2 - 1);
		}
	...
	for (int t = 0; t < lent0; t++) {
		...
		//  	cout << endl;
		for (int k = 1; k < NUM_LAYER; k++)
			sigm(sample_mem[k], bh[k], weights_up[k], sample_mem[k-1],
		...
		for (int j = 0; j < nodes_layer[NUM_LAYER]; j++) {
			...
			for (int i = 0; i < nodes_layer[NUM_LAYER-1]; i++)
				...
				weights_up[NUM_LAYER][j*nodes_layer[NUM_LAYER-1] + i] * sample_mem[NUM_LAYER-1][i];
		}
		for (int j = 0; j < nodes_layer[NUM_LAYER]; j++)
